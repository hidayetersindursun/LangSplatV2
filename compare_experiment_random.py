...
def compare(args):
    ...
    # --- SELECT RANDOM 5 CAMERAS ---
    if len(test_cameras) > 5:
        selected_cameras = random.sample(test_cameras, 5)
    else:
        selected_cameras = test_cameras

    print(f"Toplam {len(selected_cameras)} kare karşılaştırılacak (Rastgele seçildi)...")
    
    metrics = {
        "Baseline_PSNR": [], "Ours_PSNR": [],
    }

    out_dir = "comparison_results_random"
    os.makedirs(out_dir, exist_ok=True)

    for idx, view in enumerate(tqdm(selected_cameras)):
        ...
        # Image name based on camera name to track which one it is
        cv2.imwrite(os.path.join(out_dir, f"compare_{view.image_name}.png"), grid)

    print("\n" + "="*30)
    print("      SONUÇ RAPORU (RANDOM SAMPLE)      ")
    print("="*30)
